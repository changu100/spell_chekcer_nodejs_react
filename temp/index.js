...
function listener(index) {
  ...
  if (f_dspell[index] == 0) f_dspell[index] = 1;
  else f_dspell[index] = 0;
}
...
function spellTimer(gametime, btn, index, now) {
  ...
  //   let table = document.body.querySelector(".tables");
  //   // if(table.querySelector(".shadows")){
  //     table.removeChild(table.querySelector(".shadows"));
  // }

  btn.className = "dspell shadows";

  // shade.style.transform = "rotate(-"+(timer*360/300)+"deg)";
  // shade.style.position = "absolute";
  // shade.style.top = btn.getBoundingClientRect().top+"px";
  // shade.style.left = btn.getBoundingClientRect().left+"px";
  // console.log(btn.getBoundingClientRect().top);
  // console.log(btn.getBoundingClientRect().left);

  // console.log(shade.style.top);
...
function dateToString(timer, display = 111, interval = ":") {
  ...
  if (display / 100 >= 1) {
    string +=
      parseInt(timer / 3600) >= 10
        ? parseInt(timer / 3600).toString(10)
        : "0" + parseInt(timer / 3600).toString(10);
    string += interval;
  }
  if ((display / 10) % 10 >= 1) {
    string +=
      parseInt(timer / 60) % 60 >= 10
        ? (parseInt(timer / 60) % 60).toString(10)
        : "0" + (parseInt(timer / 60) % 60).toString(10);
    string += interval;
  }
  if (display % 10 >= 1) {
    string +=
      timer % 60 >= 10
        ? (timer % 60).toString(10)
        : "0" + (timer % 60).toString(10);
  }
  return string;
}
